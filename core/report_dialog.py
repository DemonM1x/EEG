...
class ReportConfigDialog(QDialog):
    ...
    def init_ui(self):
        ...
        patient_group = QGroupBox("Информация о пациенте (опционально)")
        patient_layout = QFormLayout()

        self.patient_name_edit = QLineEdit()
        self.patient_name_edit.setPlaceholderText("Иванов Иван Иванович")
        ...
        self.patient_id_edit.setPlaceholderText("ID-12345")
        patient_layout.addRow("ID пациента:", self.patient_id_edit)
        ...
        comments_label = QLabel("Дополнительные комментарии:")
        report_layout.addWidget(comments_label)

        self.comments_edit = QTextEdit()
        self.comments_edit.setMaximumHeight(100)
        self.comments_edit.setPlaceholderText("Введите дополнительные комментарии к исследованию...")
        report_layout.addWidget(self.comments_edit)
        ...
        self.generate_name_button = QPushButton("🔄 Сгенерировать имя")
        self.generate_name_button.clicked.connect(self.generate_filename)
        auto_name_layout.addWidget(self.generate_name_button)
        ...
        self.preview_button = QPushButton("👁️ Предварительный просмотр")
...
{info['comments'] if info['comments'] else 'Нет комментариев'}
    ...
    def get_report_info(self):
        ...
        if self.patient_name_edit.text().strip():
            patient_info['name'] = self.patient_name_edit.text().strip()

        if self.patient_age_spin.value() > 0:
            patient_info['age'] = self.patient_age_spin.value()

        if self.patient_gender_combo.currentText() != "Не указан":
            patient_info['gender'] = self.patient_gender_combo.currentText()

        if self.patient_id_edit.text().strip():
            patient_info['id'] = self.patient_id_edit.text().strip()

        patient_info['examination_date'] = self.examination_date.date().toString("dd.MM.yyyy")

        if self.doctor_name_edit.text().strip():
            patient_info['doctor'] = self.doctor_name_edit.text().strip()
